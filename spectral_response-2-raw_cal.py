...
def spectrum_process(raw_info_dict, wavelength_queue, wavelength_maxDN_queue, dk_img):
    ...
    while wavelength_queue.qsize() :
        ...
        if np.max(raw_data) >  12000 :
            print(current_wavelength + '波段数据灰度值大于12000 疑似饱和')
        # 求平均值
        ...
        # 方式四 特定区域求和
        img_mean[img_mean < 40] = 0
        img_mean = np.reshape(img_mean, (raw_info_dict['raw_height'], raw_info_dict['raw_width']))
        img_mean = img_mean[505:523, 505:523]  # 特定区域选取
        img_sum = np.sum(img_mean)  # 所有像素求和
        img_maxdn_dict = {'wavelength':current_wavelength, 'max_dn':img_sum}
        wavelength_maxDN_queue.put(img_maxdn_dict) 
        time.sleep(0.1)
        ...
        # 输出
        # fout = 'Spectral_Response-'+ raw_info_dict['channel_name'] + '-' + current_wavelength + '.raw'
        ...
        print(f'{current_wavelength}波长点文件输出 图像灰度值{round(img_sum,2)}')
...
if __name__ == "__main__":
    ...
    raw_info_dict = {'raw_width':1024,
                    'raw_height':1030,
                    'spectral_path': f'e:/sl/{channel_name}',
